...
`llmwiki-cli` is a single Rust binary CLI for managing a Karpathy-style LLM Wiki (markdown files + JSONL embeddings, no database). It ships with a bundled skill that auto-installs into the host agent's skill directory.
...
```bash
cargo build                          # debug build (also runs build.rs to generate skills/references/schema.json)
...
src/
  ...
  skills/                  # skill bundle embedded via rust-embed
  error.rs                 # WikiError thiserror enum
  lib.rs                   # module exports

skills/                    # source of truth for the bundled skill
  SKILL.md                 # hub — written to ~/.agents/skills/wiki/SKILL.md by install-skill
  wiki-{setup,config,...}.md   # inline sub-skills served via `llmwiki-cli skill get <topic>`
  references/schema.json   # generated by build.rs (see below)

build.rs                   # generates skills/references/schema.json from Config.
...
1. `skills/references/schema.json` — for the `Config` type.

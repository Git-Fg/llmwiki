...
**Schema root contract — `additionalProperties: false`**: Both build.rs duplicates carry `#[serde(deny_unknown_fields)]`. Schemars reads the serde attribute and emits `"additionalProperties": false` at the schema root natively (verified v0.3.22; see schemars docs at https://docs.rs/schemars). The same attribute is added to the real structs in `src/` for documentation of intent — harmless at runtime because both only derive `Serialize`, never `Deserialize`.

**Drift protection**: `tests/doctor_test.rs::doctor_json_output_validates_against_schema` asserts two things every CI run: (1) the on-disk schema validates against a real `wiki doctor --json` run against a mocked NIM, AND (2) the schema's `properties` key set matches a hard-coded canonical 15-key set. The keys check is the load-bearing half — `jsonschema::is_valid` alone cannot catch drift between `src/cli/doctor.rs::DoctorReport` and the `build.rs` duplicate, because the real struct's output would still satisfy a schema that silently lost a field.

**Maintainer invariant**: When you change either `src/core/config.rs::Config` / `src/cli/doctor.rs::DoctorReport` or the corresponding build.rs duplicate, you MUST change both. The drift test catches forgetting. Build also re-runs because `build.rs` declares `cargo:rerun-if-changed=src/cli/doctor.rs` (and `src/core/config.rs`).

## Cargo.lock (v0.3.20+) and platform-specific locks (v0.3.21+)

`Cargo.lock` is **tracked** in git (un-ignored in v0.3.20). However, cargo's lockfile is **platform-specific** — it pins deps like `windows-sys` to a Linux target that does not exist on macOS dev boxes (and vice versa). The macOS-generated lock fails `cargo check --locked` on the Linux CI runner.

**Current workaround (v0.3.21)**: `msrv-check` runs `cargo check --all-targets` WITHOUT `--locked`. Resolution + typecheck are still verified under rustc 1.88; only exact dep reproducibility is skipped.

**Forward fix (v0.3.22)**: `.github/workflows/lockfile-update.yml` runs weekly Monday 06:17 UTC on `ubuntu-latest`, executes `cargo update --workspace`, and opens a PR if `Cargo.lock` changed. The merged PR will then be the Linux-native lockfile that `msrv-check` can verify with `--locked`. Until that workflow's first merge, `msrv-check` stays `--locked`-less.

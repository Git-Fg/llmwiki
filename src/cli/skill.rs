...
pub fn run(args: SkillArgs) -> Result<(), WikiError> {
    match args.cmd {
        None | Some(SkillCmd::Show { topic: None }) | Some(SkillCmd::Get { topic: None }) => {
            print!("{}", skills::hub());
        }
        Some(SkillCmd::Show { topic: Some(t) }) | Some(SkillCmd::Get { topic: Some(t) }) => {

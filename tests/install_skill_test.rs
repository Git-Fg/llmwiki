...
fn install_skill_global_writes_hub_only() {
    ...
    // v0.3.36+: install path is `~/.agents/skills/llmwiki/` (hard cut
    // from `wiki/`). Only the hub is installed; sub-skills are served
    // at runtime via `llmwiki-cli skill get <topic>` from the embedded
    // binary.
    assert!(exists().eval(&agents_dir.join("llmwiki/SKILL.md")));

    // Sanity: no stale sub-skill directory exists from a prior install.
    assert!(
        !agents_dir.join("llmwiki").join("SETUP").exists(),
...
fn install_skill_workspace_writes_local_hub_only() {
    ...
    assert!(exists().eval(&workspace.join(".agents/skills/llmwiki/SKILL.md")));
}

#[test]
fn install_skill_install_path_overrides_default() {
    // v0.3.30: `--install-path <dir>` lets users target a specific host's
    // skills directory (e.g. `~/.claude/skills/llmwiki` for Claude Code).
    let tmp = tempfile::tempdir().unwrap();
    let custom = tmp.path().join("custom-host/skills/llmwiki");

...
//! `WIKI_ROOT_CONFIG`, current directory). They are serialized via a
//! process-wide mutex to avoid cross-test contamination.

use llmwiki_cli::core::registry::Registry;
use std::sync::Mutex;

/// Global mutex serializing all tests in this file.
static TEST_LOCK: Mutex<()> = Mutex::new(());

/// Write a wiki-root.toml to `path` with the given TOML body.
fn write_registry(path: &std::path::Path, body: &str) {
...
fn with_home_and_cwd<F, R>(home: &std::path::Path, cwd: &std::path::Path, f: F) -> R
where
    F: FnOnce() -> R,
{
...
fn without_wiki_root_config<F, R>(f: F) -> R
where
    F: FnOnce() -> R,
{
